...
const TasksPage = () => {
  ...
  const debouncedFetchTasksByBounds = debounce((bounds) => {
    setDistanceFilter("");
    fetchTasksByBounds(bounds);
  }, 500);

  const handleApply = async (task) => {
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/tasks/${task.id}/applications`, {
        tutorId: user.id,
  ...
  useEffect(() => {
    ...
    axios.get(`${import.meta.env.VITE_API_BASE_URL}/applications/my_applications`, {
      headers: { Authorization: `Bearer ${token}` },
    })
        .then(res => {
          const appliedIds = new Set(res.data.map(app => app.taskId));
  ...
  const filterTasks = () => {
    ...
    if (budgetFilter) {
      ...
      tempFiltered = tempFiltered.filter(t => {
        const budgetNum = typeof t.budget === "number" ? t.budget : 0;
        return budgetNum >= min && (max ? budgetNum <= max : true);
      });
    }
          ...
          <div className="flex flex-col md:flex-row gap-6 h-[700px]">
                      ...
                      >
                        <Card className="h-full flex flex-col bg-background/70 backdrop-blur-lg border border-blue-400/30 shadow-lg rounded-2xl hover:shadow-2xl transition-shadow duration-300">
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                  Posted by User ID: {task.userId} • {task.createdAt}
                                </p>
                              </div>
                          ...
                          <CardContent className="py-2 flex-grow">
                            <p className="text-muted-foreground text-sm mb-4 leading-relaxed whitespace-pre-line break-words">
                              {task.description}
                            </p>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                              <div className="flex items-center text-muted-foreground">
                                <DollarSign className="h-4 w-4 text-blue-400 mr-2" />
                                <span>{typeof task.budget === "number" ? `$${task.budget}` : task.budget}</span>
                              </div>
                              <div className="flex items-center text-muted-foreground">
            ...
            <div className="relative flex-1 rounded-xl overflow-hidden shadow-2xl">
              <TaskMapView
                  ...
                  onBoundsChange={debouncedFetchTasksByBounds}
              />
            </div>
          </div>
        </div>
              ...
              <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    ...
                    <Button onClick={() => handleApply(selectedTask)}>
                      Submit Application
                    </Button>
                  </div>
                </div>
